<template>
  <div class="khachhang-list-card animate-slide-up p-0">
    <div class="card-header d-flex justify-content-between align-items-center px-4 pt-4 pb-2">
      <h3 class="list-title m-0">Danh sách khách hàng</h3>
    ...
    <div v-else class="table-responsive">
      <table class="table khachhang-table mb-0">
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="khachHang in store.khachHangs" :key="khachHang._id" class="table-row-hover">
            <td>{{ khachHang.email }}</td>
            <td>{{ khachHang.hoVaTen }}</td>
            <td>{{ khachHang.sdt }}</td>
            <td class="action-buttons text-center">
              <button class="action-btn text-edit-btn me-2" @click="$emit('edit', khachHang)" v-tooltip="'Sửa khách hàng'">Sửa</button>
              <button class="action-btn text-delete-btn" @click="store.deleteKhachHang(khachHang._id)" v-tooltip="'Xóa khách hàng'">Xóa</button>
            </td>
          </tr>
        </tbody>
      </table>
      <nav class="mt-4">
        <ul class="pagination justify-content-center khachhang-pagination">
          <li class="page-item" :class="{ disabled: store.pagination.page === 1 }">
            <a class="page-link" @click="store.fetchKhachHangs(store.pagination.page - 1)"><i class="bi bi-chevron-left"></i> Trước</a>
          </li>
          <li class="page-item" v-for="n in Math.ceil(store.pagination.total / store.pagination.limit)" :key="n" :class="{ active: n === store.pagination.page }">
            <a class="page-link" @click="store.fetchKhachHangs(n)">{{ n }}</a>
          </li>
          <li class="page-item" :class="{ disabled: store.pagination.page === Math.ceil(store.pagination.total / store.pagination.limit) }">
            <a class="page-link" @click="store.fetchKhachHangs(store.pagination.page + 1)">Sau <i class="bi bi-chevron-right"></i></a>
          </li>
        </ul>
      </nav>
    </div>
...
.khachhang-pagination .page-item .page-link {
...
.khachhang-pagination .page-item.active .page-link,
.khachhang-pagination .page-item .page-link:hover {
  background: linear-gradient(90deg, #6366f1 0%, #60a5fa 100%);
  color: #fff;
  border-color: transparent;
  box-shadow: 0 2px 10px rgba(99, 102, 241, 0.4);
}
.khachhang-pagination .page-item.disabled .page-link {
  background: #1A202C;
  color: #6b7280;
  border-color: #2D3748;
  cursor: not-allowed;
...
@media (max-width: 992px) {
  .khachhang-table th, .khachhang-table td {
