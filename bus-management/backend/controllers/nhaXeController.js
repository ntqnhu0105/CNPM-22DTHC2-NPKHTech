const { getAllNhaXe, createNhaXe, updateNhaXe, deleteNhaXe: deleteNhaXeService } = require('../services/nhaXeService');
...
const deleteNhaXe = async (req, res, next) => {
  try {
    ...
    const result = await deleteNhaXeService(req.params.id);
    res.json(result);
  } catch (err) {
    next(err);
  }
